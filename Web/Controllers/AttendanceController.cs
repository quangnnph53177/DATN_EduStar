...
namespace Web.Controllers
{
    public class AttendanceController : Controller
    {
        ...
        [HttpGet]
        public IActionResult StudentCheckIn()
        {
            var model = new CheckInByFaceDto(); 
            return View(model);
        }
        [HttpPost]
        // [Authorize(Roles = "Student")]
        public async Task<IActionResult> StudentCheckIn([FromForm] CheckInByFaceDto model)
        {
            ...
            try
            {
                ...
                content.Add(new StringContent(model.AttendanceId.ToString()), "AttendanceId");
                content.Add(new StreamContent(model.FaceImage.OpenReadStream()), "FaceImage", model.FaceImage.FileName);

                var response = await client.PostAsync("api/Attendance/checkin-by-face", content);
                ...
                {
                    var error = await response.Content.ReadAsStringAsync();
                    return Json(new { success = false, message = "Điểm danh thất bại: " + error });
                }
            }
            catch (Exception)
            {
